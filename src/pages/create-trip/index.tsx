...
export function CreateTripPage() {
  ...
  const [emaisToInvite, setEmailsToInvite] = useState<string[]>([
    "aleatorio@gmail.com",
  ]);
  ...
  function addNewEmailToInvite(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (emaisToInvite.includes(email)) {
      return;
    }
    setEmailsToInvite([...emaisToInvite, email]);
    event.currentTarget.reset();
  }

  function removeEmailToInvite(email: string) {
    setEmailsToInvite(emaisToInvite.filter((item) => item !== email));
  }
          ...
          {isGuestsInputOpen && (
            <div className="h-16 bg-zinc-900 px-4 rounded-xl flex items-center shadow-shape gap-3">
              ...
              >
                ...
                {emaisToInvite.length > 0 ? (
                  <span className="text-zinc-400">
                    {emaisToInvite.length} pessoa(s) convidada(s)
                  </span>
                ) : (
